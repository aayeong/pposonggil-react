...
//     // home으로 이동 (홈화면 재사용해서 클릭한 장소로 마크업 및 장소 정보 출력)
//     navigate(`/home`);
//   };
...
// // test용
// // const select=useRecoilValue(searchPlace);
// // console.log("선택 장소 정보: ", select.address);
...
const ResultContainer = styled.div`
  ...
  height: 90%;
  padding-bottom: 70px;
  transition: all 0.3s ease; /* 부드러운 전환을 위한 트랜지션 */
`;
...
function SearchPlace() {
  ...
  const [searchResults, setSearchResults] = useState([]); // 검색 결과(자동완성 기능)
  const [searchHistory, setSearchHistory] = useState([]); // 검색 기록(로컬스토리지)
  const [showMap, setShowMap] = useState(false); // 지도를 보여줄지 여부
  ...
  if (showMap) {
    ...
    // useRecoilValue로부터 받아온 값을 콜백 내부에서 사용
    const select = searchPlace; // useRecoilValue 대신 직접 상태를 가져옴
    console.log("선택 장소 정보: ", select.address);

    if (select.address) {
      const geocoder = new kakao.maps.services.Geocoder();
      geocoder.addressSearch(select.address, (result, status) => {
        if (status === kakao.maps.services.Status.OK) {
...
}, [showMap, searchPlace]); // 의존성 목록에 searchPlace 추가
